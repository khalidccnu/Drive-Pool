...
export default function Sidebar() {
  ...
  return (
    <aside
      className={`flex h-screen flex-none flex-col border-r border-dp-border bg-dp-sidebar transition-all duration-200 ${collapsed ? "w-14" : "w-60"}`}
    >
      {/* Logo + collapse toggle */}
      <div className={`flex items-center border-b border-dp-border px-3 py-[14px] ${collapsed ? "justify-center" : "justify-between"}`}>
        <div className={`flex items-center gap-2.5 ${collapsed ? "justify-center" : ""}`}>
          ...
          {!collapsed && <span className="text-sm font-semibold text-dp-text">DrivePool</span>}
        </div>
        {!collapsed && (
          <button
            onClick={() => setCollapsed(true)}
            className="flex h-6 w-6 items-center justify-center rounded-md text-dp-text3 transition hover:bg-dp-hover hover:text-dp-text"
      ...
      {/* Nav */}
      <nav className={`flex-1 space-y-0.5 py-4 ${collapsed ? "px-1.5" : "px-3"}`}>
        {!collapsed && (
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 rounded-lg transition-all ${collapsed ? "justify-center px-0 py-2.5" : "px-3 py-2.5"} text-sm ${
                isActive
            ...
            >
              ...
              {!collapsed && (
                <>
                  {item.label}
                  {isActive && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-orange-400" />}
                </>
              )}
            </Link>
          );
